...
let rec transpose matrix =
    match matrix with // matrix is a list<list<'a>>
    | row :: rows -> // case when the list of rows is non-empty
        match row with // rows is a list<'a>
...
let setBit number index = number ||| (1 <<< index)
...
let solvePartOne =
    ...
    |> List.rev
    |> List.map (fun l ->
        ((Seq.filter (fun c -> c = '0') l |> Seq.length), (Seq.filter (fun c -> c = '1') l |> Seq.length)))
    |> List.fold
        (fun ((gamma, epsilon), i) (zeros, ones) ->
            ((if ones > zeros then
                  (setBit gamma i, epsilon)
              else
                  (gamma, setBit epsilon i))),
            i + 1)
        ((0, 0), 0)
    |> fst
